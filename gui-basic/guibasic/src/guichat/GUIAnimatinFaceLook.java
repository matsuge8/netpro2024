...
class GUIAnimatinFaceLook {// 顔のオブジェクト
	...
	public void makeMouth(Graphics g, int mouseSize ,int mouseUpDown) {
		int xMiddle = xStart + h / 2;
		int yMiddle = yStart + 3 * w / 4;
		g.drawLine(xMiddle - mouseSize / 2, yMiddle, xMiddle,
				yMiddle + mouseUpDown);
        g.drawLine(xMiddle, yMiddle + mouseUpDown, xMiddle + mouseSize / 2,
				yMiddle);
	}
}// FaceObj End