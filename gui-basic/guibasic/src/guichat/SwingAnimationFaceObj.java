	...
	class BallRim {
		...
		void move() {
			...
			if ((xDir >0) && (x+this.radius*2 >= w)) {
				xDir = -1*xDir;
				setCollisionText("左に進むぞ",3);
			}
			else if ((xDir <0) && (x <= 0)) {
				xDir = -1*xDir;
				setCollisionText("右に行くぞ",6);
			}else if (xDir >0) {
				x = x + 10;
			} else {
				x = x - 10;
			...
			if ((yDir >0) && (y+this.radius*2 >= h)) {
				yDir = -1*yDir;
				setCollisionText("上にあがるぞ",3);
			}
			if ((yDir <0) && (y <= 0)) {
				yDir = -1*yDir;
				setCollisionText("下に行きまーす",6);
			}
	...
	class FaceObj {
		...
		public void makeEyes(Graphics g, int eyeSize) {
			...
			g.fillArc(xStart + (h * 2/ 7) , yStart + (w * 1 / 3), eyeSize,
					eyeSize, 150, 180);
			g.fillArc(xStart + (h * 4/ 7) , yStart + (w * 1 / 3), eyeSize,
					eyeSize, 210, 180);
		...
		public void makeMouth(Graphics g, int mouseSize) {
			...
			int yMiddle = yStart + 3*w/4 + 2;
			g.drawLine(xMiddle - mouseSize / 2, yMiddle, xMiddle , yMiddle -8);
			g.drawLine(xMiddle , yMiddle -8, xMiddle + mouseSize / 2 , yMiddle );
		}
	}//FaceObj End
